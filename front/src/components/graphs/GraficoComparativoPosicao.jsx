...
export default function GraficoComparativoPosicao({ dadosPorCidade, cidadesSelecionadas, cores }) {
  const svgRef = useRef();
  ...
  const desenhar = useCallback(() => {
    ...
    const height = 260;
    const margin = { top: 28, right: 20, bottom: isMobile ? 60 : 44, left: 48 };
    ...
    const allPositions = cidadesSelecionadas.flatMap(c =>
      (dadosPorCidade[c] || []).map(d => d.posicao).filter(Boolean)
    );
    ...
    const line = d3.line()
      .x(d => x(d.label))
      .y(d => y(d.posicao))
      .defined(d => d.posicao != null)
      .curve(d3.curveMonotoneX);

    cidadesSelecionadas.forEach(cidade => {
      const dados = (dadosPorCidade[cidade] || []).filter(d => todosLabels.includes(d.label) && d.posicao);
  ...
  return (
    <div className="border rounded-lg p-4">
      <p className="text-sm font-medium text-[#231f20] mb-1">Evolução Comparativa da Posição</p>
